...
__global__ void d_exp_reduction(float* matrix, int height, int width, float* result)
{
  ...
  for (int i = 0; i < height / blockDim.x + 1 && offset < height; ++i, offset += blockDim.x)
    local_sum += matrix[offset] = EXP_FUNC(matrix[offset]);
...
extern "C" void m_exp_reduction(float* matrix, int height, int width, float* result)
{
  d_exp_reduction<<<width, REDUCTION_THREADS, REDUCTION_THREADS * sizeof(float)>>>(matrix, height, width, result);
}
...
{
  d_softmax<<<(ALIGN32(height) * width) / EXP_THREADS + 1, SOFTMAX_THREADS>>>(matrix, height, width, exp_sum);
}
